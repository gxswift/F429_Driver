...
void GT9XX_Reset(void)
{
	...
	// ³õÊ¼»¯Òı½Å×´Ì¬
	HAL_GPIO_WritePin(GPIOA,GPIO_PIN_15,0); 	// ½«INTÀ­µÍ
	HAL_GPIO_WritePin(GPIOI,GPIO_PIN_11,1);	// ½«RSTÀ­¸ß
	T_Delay(10000);	
	
	// ¿ªÊ¼Ö´ĞĞ¸´Î»
	//	INTÒı½Å±£³ÖµÍµçÆ½²»±ä£¬½«Æ÷¼şµØÖ·ÉèÖÃÎª0XBA/0XBB
	HAL_GPIO_WritePin(GPIOI,GPIO_PIN_11,0);			// À­µÍ¸´Î»Òı½Å£¬´ËÊ±Ğ¾Æ¬Ö´ĞĞ¸´Î»
	T_Delay(350000);										// ÑÓÊ±
	HAL_GPIO_WritePin(GPIOI,GPIO_PIN_11,1);			// À­¸ß¸´Î»Òı½Å£¬¸´Î»½áÊø
	T_Delay(350000);										// ÑÓÊ±
...
uint8_t GT9XX_WriteHandle (uint16_t addr)
{
	...
	IIC_Touch_Start();	// Æô¶¯IICÍ¨ĞÅ
	
	if( IIC_Touch_WriteByte(GT9XX_IIC_WADDR) == 1 ) //Ğ´Êı¾İÖ¸Áî
	{
		if( IIC_Touch_WriteByte((uint8_t)(addr >> 8)) == 1 ) //Ğ´Èë16Î»µØÖ·
		{
...
uint8_t GT9XX_WriteData (uint16_t addr,uint8_t value)
{
	...
	if( GT9XX_WriteHandle(addr) == SUCCESS)	//Ğ´ÈëÒª²Ù×÷µÄ¼Ä´æÆ÷
	{
		if (IIC_Touch_WriteByte(value) != 1) //Ğ´Êı¾İ
		{
...
uint8_t GT9XX_WriteReg (uint16_t addr, uint8_t cnt, uint8_t *value)
{
	...
	if( GT9XX_WriteHandle(addr) == SUCCESS) //Ğ´ÈëÒª²Ù×÷µÄ¼Ä´æÆ÷
	{
		for(i = 0 ; i < cnt; i++)		// ¼ÆÊı
		{
			IIC_Touch_WriteByte(value[i]);	// Ğ´ÈëÊı¾İ
		}					
		IIC_Touch_Stop();		// Í£Ö¹IICÍ¨ĞÅ
		status = SUCCESS;		// Ğ´Èë³É¹¦
	}
	else
	{
...
uint8_t GT9XX_ReadData (uint16_t addr, uint8_t cnt, uint8_t *value)
{
	...
	if( GT9XX_WriteHandle(addr) == SUCCESS) 	//	Ğ´ÈëÒª²Ù×÷µÄ¼Ä´æÆ÷
	{
		...
		if (IIC_Touch_WriteByte(GT9XX_IIC_RADDR) == 1)	// ·¢ËÍ¶ÁÃüÁî
		{	
			for(i = 0 ; i < cnt; i++)	// ¼ÆÊı
			{
				if (i == (cnt - 1))
				...
				{
					value[i] = IIC_Touch_ReadByte(1);	// ·¢ËÍÓ¦´ğĞÅºÅ
				}
			}					
			IIC_Touch_Stop();	// Í£Ö¹IICÍ¨ĞÅ
			status = SUCCESS;
...
void GT9XX_ReadCfg(void)
{
	...
	GT9XX_ReadData (GT9XX_CFG_ADDR,184,GT9XX_Cfg);	// ¶ÁÅäÖÃĞÅÏ¢
	for(i=0;i<184;i++)
	{	
		if( (i%10 == 0) && (i>0) )			
		{
...
//	GT9XX_SendCfg();	
	...
	GT9XX_ReadData (GT9XX_ID_ADDR,11,GT9XX_Info);		// ¶Á´¥ÃşÆÁICĞÅÏ¢
	GT9XX_ReadData (GT9XX_CFG_ADDR,1,&cfgVersion);	// ¶Á´¥ÃşÅäÖÃ°æ±¾
